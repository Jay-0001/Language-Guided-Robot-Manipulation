...
def run_grounding_on_image(model, image_pil, caption, box_threshold=0.5, text_threshold=0.4, device="cuda"):
    """
    A thin wrapper around get_boxes() that:
    - accepts a PIL image
    - converts it to tensor
    - runs GroundingDINO
    - returns the highest-confidence predicted box in absolute pixel coords
    """
